...
Methods only annotated with @Outgoing do not support acknowledgement as they don’t receive an input Message.
...
The [hotspot=initFoodOrder file=0]`receiveFoodOrder()` method in Kitchen microservice annotated with [hotspot=ackPostProcessing file=0]`POST_PROCESSING` acknowledgement strategy indicates that the incoming message is acknowledged when the outgoing message is acknowledged. i.e. when the emitted data is acknowledged. In case, the service goes down when the message is being processed, the kafka will resend the message and the message will not be lost.
