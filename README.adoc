...
= Acknowledging asynchronous messages using MicroProfile Reactive Messaging acknowledgement strategies
...
You'll update the `Kitchen`, `Bar`, and `Order` microservices to use MicroProfile Reactive Messaging acknowledgement strategies for message acknowledgement. These microservices run on Open Liberty.
...
MicroProfile Reactive Messaging Acknowledgement ensures that in case of any system failure, no messages are lost. Messaging system like Apache Kafka differentiates between when a message is being delivered to a system as opposed to been processed completely. If the system goes down, any messages which have been sent but not processed completely can then be sent again. Reactive messaging applications implement this using message acknowledgements.
...
Different defaults and supported acknowledgement for each supported signature could be found https://download.eclipse.org/microprofile/microprofile-reactive-messaging-1.0/microprofile-reactive-messaging-spec.html#_message_acknowledgement[here^] in this table.

NOTE : Method only annotated with @Outgoing do not support acknowledgement as they don’t receive an input Message.

=== MicroProfile Reactive Messaging NONE Acknowledgement strategy

If the acknowledgement strategy is set to `NONE`, no acknowledgment will be performed. The `NONE` strategy indicates that the incoming message is not acknowledged and the acknowledgment of the outgoing message would not acknowledge the incoming message anymore. The `NONE` strategy may be used for protocols that do not support message acknowledgment.
...
The [hotspot=initBevOrder file=0]`receiveBeverageOrder()` method in Bar microservice annotated with [hotspot=ackPreProcessing file=0]`PRE_PROCESSING` acknowledgement strategy matches the signature 
...
Based on the above mentioned table, the default acknowledgement strategy would be `POST_PROCESSING`. It means that the messages will be acknowledged when the annotated method executes completely. However when we change the strategy to `PRE_PROCESSING`, the incoming message will be acknowledged even before the annotated method is called. It also means that the acknowledgment of the outgoing message would not acknowledge the incoming message anymore, as it’s already acknowledged. So if the service goes down when the message is being processed, the kafka won't resend the message and the message would be lost.
...
The [hotspot=initFoodOrder file=0]`receiveFoodOrder()` method in Kitchen microservice annotated with [hotspot=ackPostProcessing file=0]`POST_PROCESSING` acknowledgement strategy indicates that the incoming message is acknowledged when the outgoing message is acknowledged. i.e. when the emitted data is acknowledged. So if the service goes down when the message is being processed, the kafka will resend the message and the message will not be lost.

Check the https://download.eclipse.org/microprofile/microprofile-reactive-messaging-1.0/microprofile-reactive-messaging-spec.html#_message_acknowledgement[specification^] to see whether reactive messaging does `PRE_PROCESSING` or `POST_PROCESSING` message acknowledgements for your method. If it does `PRE_PROCESSING` consider changing to `POST_PROCESSING` if you don't want message to be lost.
...
The [hotspot=updateStatusMethod file=1]`updateStatus()` method in Status microservice annotated with [hotspot=ackManual file=1]`MANUAL` acknowledgement strategy 
indicates that the incoming message acknowledgement is managed by the user code. The [hotspot=manualAckUser file=1]`order.ack()` method acknowledges the message once the method completes all it's processing.
